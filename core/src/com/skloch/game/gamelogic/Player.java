...
public class Player {
  ...
  public Player(String avatar) {
    ...
    runningAnimation.add(
        new Animation<TextureRegion>(0.1f, playerAtlas.findRegions(avatar + "_walk_back"),
            Animation.PlayMode.LOOP),
        new Animation<TextureRegion>(0.1f, playerAtlas.findRegions(avatar + "_walk_right"),
            Animation.PlayMode.LOOP),
        new Animation<TextureRegion>(0.1f, playerAtlas.findRegions(avatar + "_walk_front"),
            Animation.PlayMode.LOOP),
        new Animation<TextureRegion>(0.1f, playerAtlas.findRegions(avatar + "_walk_left"),
            Animation.PlayMode.LOOP));
    // Load idle animation
    idleAnimation.add(
   ...
   * 'slide' off of it. Also updates the player's animation Also locates the nearest object after
   * moving, which can be used to trigger events
   *
  ...
  private float distanceFrom(GameObject object) {
    return (float) Math.sqrt(
        (Math.pow((centreX - object.centreX), 2) + Math.pow((centreY - object.centreY), 2)));
  }
